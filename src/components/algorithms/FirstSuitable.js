  ...
  startAlgorithm = () => {
    ...
    details.forEach((detail, index) => {
      ...
      for (var canvasIndex = 0; canvasIndex < canvases.length; canvasIndex++) {
        ...
        if (
          detail.width > currentCanvas.width ||
          detail.height > currentCanvas.height
        ) {
        ...
        let suitableDetailForWidth = getSuitableDetailsForWidth(
          currentCanvas.details,
          currentCanvas.verifiableDetailsOnWidth,
          detail,
          currentCanvas
        );
        let suitableDetailForHeight = getSuitableDetailsForHeight(
          currentCanvas.details,
        ...
        console.log(
          `[INFO] есть подходящая по ширине ${!!suitableDetailForWidth}`
        );
        console.log(
          `[INFO] есть подходящая по высоте ${!!suitableDetailForHeight}`
        );
        ...
        if (suitableDetailForWidth) {
          let currentDetail = {
            ...
            points: {
              bottomRight: getBottomRightPointBySizeAndBottomLeftPoint(
                suitableDetailForWidth.points.bottomRight,
                detail
              ),
              topLeft: getTopLeftPointBySizeAndBottomLeftPoint(
                suitableDetailForWidth.points.bottomRight,
          ...
          let detailIndexToDelete = currentCanvas.verifiableDetailsOnWidth.findIndex(
            (detail) => detail.id === detailIdToDelete
          );
        ...
        if (suitableDetailForHeight) {
          let currentDetail = {
            ...
            points: {
              bottomRight: getBottomRightPointBySizeAndBottomLeftPoint(
                suitableDetailForHeight.points.topLeft,
                detail
              ),
              topLeft: getTopLeftPointBySizeAndBottomLeftPoint(
                suitableDetailForHeight.points.topLeft,
          ...
          let detailIndexToDelete = currentCanvas.verifiableDetailsOnHeight.findIndex(
            (detail) => detail.id === detailIdToDelete
          );
          currentCanvas.verifiableDetailsOnHeight.splice(
            detailIndexToDelete,
            1
          );
      ...
      if (isAdded) {
        console.log(
          "[OPERATION] исключение некорректных деталей из проверочных"
        );
        ...
        detailsIndexToDeleteOnWidth.forEach((index) =>
          usedCanvas.verifiableDetailsOnWidth.splice(index, 1)
        );
        detailsIndexToDeleteOnHeight.forEach((index) =>
          usedCanvas.verifiableDetailsOnHeight.splice(index, 1)
        );
      }
      ...
      if (!isBadDetail && !isAdded) {
        ...
        if (
          detail.width > newCanvas.width ||
          detail.height > newCanvas.height
        ) {
          badDetails.push({
            id: detail.id,
            width: Math.round(detail.width / factor),
...
export default withStyles(styles)(
  connect(mapStateToProps)(withSnackbar(FirstSuitable))
);
